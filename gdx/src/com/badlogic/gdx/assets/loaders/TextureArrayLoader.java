...
/** {@link AssetLoader} for {@link TextureArray} instances. Multiple files can be specified by specifying a file name that ends
 * in the integer 0 before the file suffix (in which case a search is made for consecutive following integers, or by explicitly 
 * naming all the files in a passed {@link TextureArrayParameter}; 
 * 
 * The pixel data is loaded asynchronously. The texture is then created on the
 * rendering thread, synchronously. Passing a {@link TextureArrayParameter} to
...
public class TextureArrayLoader extends AsynchronousAssetLoader<TextureArray, TextureArrayLoader.TextureArrayParameter> {
	...
	public TextureArray loadSync (AssetManager manager, String fileName, FileHandle file, TextureArrayParameter parameter) {
		...
		TextureArray texture = new TextureArray(data);
		if (parameter != null) {
