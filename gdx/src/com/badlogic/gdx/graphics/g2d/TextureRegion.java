 ...
 * @author mzechner
 * @author Nathan Sweet */
public class TextureRegion<T extends GLTexture> {
	T texture;
	float u, v;
	float u2, v2;
	...
	public TextureRegion (T texture) {
	...
	public TextureRegion (T texture, int width, int height) {
		this.texture = texture;
	...
	/** @param width The width of the texture region. May be negative to flip the sprite when drawn.
	 * @param height The height of the texture region. May be negative to flip the sprite when drawn. */
	public TextureRegion (T texture, int x, int y, int width, int height) {
		this.texture = texture;
		setRegion(x, y, width, height);
	}

	public TextureRegion (T texture, float u, float v, float u2, float v2) {
		this.texture = texture;
		setRegion(u, v, u2, v2);
	}
	...
	public TextureRegion<T>[][] split (int tileWidth, int tileHeight) {
		int x = getRegionX();
		int y = getRegionY();
	 ...
	 * @param tileHeight a tile's height in pixels
	 * @return a 2D array of TextureRegions indexed by [row][column]. */
	public static TextureRegion<Texture>[][] split (Texture texture, int tileWidth, int tileHeight) {
		TextureRegion<Texture> region = new TextureRegion<Texture>(texture);
