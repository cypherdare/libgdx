 ...
 * 
 * @param <T> The type of GLTexture this region is a part of.
 * @author mzechner
 * @author Nathan Sweet */
public class GLTextureRegion<T extends GLTexture> {
	T texture;
	float u, v;
	float u2, v2;
	...
	public GLTextureRegion () {
	}

	/** Constructs a region the size of the specified texture. */
	public GLTextureRegion (T texture) {
	...
	public GLTextureRegion (T texture, int width, int height) {
		this.texture = texture;
	...
	/** @param width The width of the texture region. May be negative to flip the sprite when drawn.
	 * @param height The height of the texture region. May be negative to flip the sprite when drawn. */
	public GLTextureRegion (T texture, int x, int y, int width, int height) {
		this.texture = texture;
		setRegion(x, y, width, height);
	}

	public GLTextureRegion (T texture, float u, float v, float u2, float v2) {
		this.texture = texture;
		setRegion(u, v, u2, v2);
	}

	/** Sets the texture and sets the coordinates to the size of the specified texture. */
	public void setRegion (T texture) {
		this.texture = texture;
		setRegion(0, 0, texture.getWidth(), texture.getHeight());
	}
	...
	/** @return If a texture array is used, the array layer on which the region resides, otherwise 0. */
	public int getLayer (){
		return 0;
	}
	...
	/** @return If this region originated from a texture atlas and the original file name for the image ended in a number, that number.
	 * Otherwise, -1. */
	public int getIndex(){
		return -1;
	}
